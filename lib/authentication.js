...
module.exports = function(options){
  ...
  if(options.grants == 'all') options.grants = ['authorization_grant', 'implicit', 'password', 'client_credentials', 'assertion'];
  ...
  if(options.grants.indexOf('assertion') != -1) options.grant_types.push('assertion');
      ...
      backend.authenticateClient(clientCreds, function(err, authenticated) {
        ...
        if(err || !authenticated) {
          ...
          switch(req.body.grant_type) {
            case 'assertion':
              if(req.body.assertion && req.body.assertion_type){
                var params = {
                  assertion: req.body.assertion,
                  assertion_secret: req.body.assertion_secret,
                  assertion_type: req.body.assertion_type,
                  client_id: req.body.client_id
                };
                backend.grantAssertionToken(params, handleTokenResponse(res));
              } else {
...
function parseAuth(req) {
 ...
 if(token = (req.query.access_token || req.body.access_token)) {
   type = 'oauth';
 } else if((auth = req.headers['authorization']) &&
    (auth = auth.split(' ')).length > 1) {
   type = auth[0].toLowerCase();
   token = auth[1];
 }
...
function parseClientCredentials(req) {
  ...
  if(req.auth && req.auth.type == 'basic') {
    creds = base64.decode(req.auth.token).split(':');
    if(creds.length != 2) { return null }  
    else {
      clientCreds = {client_id: creds[0], client_secret: creds[1]};
    }  
...
function handleTokenResponse(res) {
  return function(err, response) {
    if(err || !response || !response.access_token) { token_error(res, 'invalid_grant', err); } 
